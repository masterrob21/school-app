                        ...
                        <table class="table-auto w-full text-left">
                            ...
                            <tbody>
                                @if (count($occupations)<1)
                                    <tr class="border-b">
                                        <td colspan="2" class="p-3 text-xl font-bold text-red-400"><h2>No occupation found.</h2></td>
                                    </tr>
                                @endif
                                @foreach ($occupations as $occupation)
                                    <tr class="border-b even:bg-gray-50 whitespace-nowrap">
                                        <td class="px-3 py-1 flex items-center space-x-1">
                                            <a href="occupations/{{$occupation->id}}/edit" class="p-2 bg-slate-400 hover:bg-slate-300 rounded">
                                            ...
                                            <form action="" method="POST" id="delete_form">
                                                @csrf
                                                @method('DELETE')
                                                <button id="{{$occupation->id}}"  class="btn_remove p-2 bg-red-400 hover:bg-red-300 rounded">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block">
                                        ...
                                        <td class="px-3 py-1 capitalize">{{ $occupation->occupation }}</td>                                    
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
