<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('New Course') }}
        </h2>
    </x-slot>

    @if (session('success'))
        <x-alert class="bg-green-400">
            {{ session('success') }}
        </x-alert>
    @endif

    <div class="py-10">
        <div class=" max-w-7xl mx-auto px-8">
            <div class="mb-4">
                <ul class="px-4 py-2 list-none bg-slate-200">
                    <li class=" inline text-lg"><a href="/courses" class="text-blue-600 hover:underline">Courses</a></li>
                    <li class="inline text-lg before:p-2 before:content-['/']">New Course</li>
                </ul>
            </div>
        </div>

        <div class="w-full sm:max-w-lg mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <x-validation-errors class="mb-4" />

                    <form method="POST" action="{{ route('courses.store') }}">
                        @csrf

                        <div >
                            <x-label for="course_code" value="{{ __('Course Code:') }}" />
                            <x-input id="course_code" class="block mt-1 w-full" type="text" name="course_code" :value="old('course_code')" required autofocus />
                        </div>

                        <div class="mt-4">
                            <x-label for="course_name" value="{{ __('Course Name:') }}" />
                            <x-input id="course_name" class="block mt-1 w-full" type="text" name="course_name" :value="old('course_name')" required autocomplete="last_name" />
                        </div>

                        <div class="mt-4">
                            <x-label for="course_description" value="{{ __('Course Description') }}" />
                            <textarea class="block mt-1 w-full" name="course_description" id="course_description" cols="30" rows="10" :value="old('course_description')" placeholder="Provide description for the course." autocomplete="description"></textarea>
                        </div>

                        <div class="mt-4">
                            <x-label for="credits" value="{{ __('Credits') }}" />
                            <x-input id="credits" class="block mt-1 w-full" type="number" name="credits" :value="old('credits')" required />
                        </div>

                        <div class="mt-4">
                            <x-button>
                                {{ __('Add Course') }}
                            </x-button>
                        </div>
                    </form>
                    
                </div>
            </div>
        </div>
    </div>
</x-app-layout>