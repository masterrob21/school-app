<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Invoice Payment') }}</h2>
    </x-slot>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('invoices.payments.update', [$invoice, $payment]) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" step="0.01" min="0.01" name="amount" id="amount" value="{{ old('amount', $payment->amount) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                @error('amount')
                            ...
                            <div>
                                ...
                                <select name="payment_mode_id" id="payment_mode_id" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                    ...
                                    @foreach($paymentModes as $mode)
                                        <option value="{{ $mode->id }}" @selected(old('payment_mode_id', $payment->payment_mode_id) == $mode->id)>{{ $mode->payment_mode }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', \Illuminate\Support\Carbon::parse($payment->payment_date)->format('Y-m-d')) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                @error('payment_date')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" name="reference_no" id="reference_no" value="{{ old('reference_no', $payment->reference_no) }}" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md focus:border-blue-500 focus:ring-blue-500" required>
                                @error('reference_no')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="flex items-center justify-end gap-4">
                            ...
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-yellow-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-700 transition">{{ __('Update Payment') }}</button>
                        </div>
                    </form>
                </div>
            </div>
