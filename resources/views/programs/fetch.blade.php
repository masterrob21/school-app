        ...
        @foreach ($programs as $program)
            <tr class="border-b even:bg-gray-50 whitespace-nowrap capitalize">
                <td class="p-3 flex items-center space-x-1">
                    <a href="programs/{{$program->id}}/edit" class="p-2 bg-slate-400 hover:bg-slate-300 rounded">
                    ...
                    <form action="" method="POST" id="delete_form">
                        @csrf
                        @method('DELETE')
                        <button id="{{$program->id}}"  class="btn_remove p-2 bg-red-400 hover:bg-red-300 rounded">
                ...
                <td class="p-3 uppercase">{{ $program->program_type }}</td>                                    
                <td class="p-3 capitalize">{{ $program->program }}</td>                                    
                <td class="p-3">{{ $program->sort_order }}</td>                                    
            </tr>
        @endforeach
    </tbody>
</table>
